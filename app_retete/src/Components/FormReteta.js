...
const FromReteta = ({retete, setRetete, ingrediente, setIngrediente}) => {
    ...
    const handleSubmit = (e) => { 
        ...
        resetForm(ingrediente, setIngrediente);
    }
...
const resetForm = (ingrediente, setIngrediente) => {
    document.getElementById('formReteta').reset();
    const key = Object.keys(ingrediente);
    // const values = Object.values(ingrediente);
    // setIngrediente({});
    console.log(ingrediente);
    key.forEach(k => {
        setIngrediente((prevIngrediente) => ({
